...
async function paymentNotification(req: Request, res: Response) {
  const body = req.body;
  let status = false;
  let msg = "Error";
  let statusCode = 500;
  ...
  try {
    ...
    if (!isNil(getOrder)) {
      ...
      if (body.fraud_status === "accept" && body.transaction_status.includes(["settlement", "capture"])) {
        getOrder.orderStatus = 2; // selesai
